...
public:
    enum {
        REQUESTVOTE = 0x37a2eba5,
        APPENDENTRIES = 0x410d586d,
        APPENDENTRIESCOMBINED = 0x5be568c9,
        EMPTYAPPENDENTRIES = 0x277ca32b,
        HELLORPC = 0x413e7706,
    };
    ...
    virtual void AppendEntries(const siteid_t& candidateId, const uint64_t& prevLogIndex, const uint64_t& prevLogTerm, const uint64_t& term, const uint64_t& commitIndex, const MarshallDeputy& cmd, uint64_t* returnTerm, bool_t* followerAppendOK, rrr::DeferredReply* defer) = 0;
    ...
    virtual void EmptyAppendEntries(const uint64_t& term, const siteid_t& candidateId, const uint64_t& leaderCommitIndex, uint64_t* returnTerm, rrr::DeferredReply* defer) = 0;
...
private:
    ...
    void __AppendEntries__wrapper__(rrr::Request* req, rrr::ServerConnection* sconn) {
        ...
        rrr::DeferredReply* __defer__ = new rrr::DeferredReply(req, sconn, __marshal_reply__, __cleanup__);
        this->AppendEntries(*in_0, *in_1, *in_2, *in_3, *in_4, *in_5, out_0, out_1, __defer__);
    }
    ...
    void __EmptyAppendEntries__wrapper__(rrr::Request* req, rrr::ServerConnection* sconn) {
        uint64_t* in_0 = new uint64_t;
        req->m >> *in_0;
        siteid_t* in_1 = new siteid_t;
        req->m >> *in_1;
        ...
        rrr::DeferredReply* __defer__ = new rrr::DeferredReply(req, sconn, __marshal_reply__, __cleanup__);
        this->EmptyAppendEntries(*in_0, *in_1, *in_2, out_0, __defer__);
    }
...
public:
    ...
    rrr::Future* async_AppendEntries(const siteid_t& candidateId, const uint64_t& prevLogIndex, const uint64_t& prevLogTerm, const uint64_t& term, const uint64_t& commitIndex, const MarshallDeputy& cmd, const rrr::FutureAttr& __fu_attr__ = rrr::FutureAttr()) {
    ...
    rrr::i32 AppendEntries(const siteid_t& candidateId, const uint64_t& prevLogIndex, const uint64_t& prevLogTerm, const uint64_t& term, const uint64_t& commitIndex, const MarshallDeputy& cmd, uint64_t* returnTerm, bool_t* followerAppendOK) {
        rrr::Future* __fu__ = this->async_AppendEntries(candidateId, prevLogIndex, prevLogTerm, term, commitIndex, cmd);
        if (__fu__ == nullptr) {
            return ENOTCONN;
        }
        rrr::i32 __ret__ = __fu__->get_error_code();
    ...
    rrr::Future* async_EmptyAppendEntries(const uint64_t& term, const siteid_t& candidateId, const uint64_t& leaderCommitIndex, const rrr::FutureAttr& __fu_attr__ = rrr::FutureAttr()) {
    ...
    rrr::i32 EmptyAppendEntries(const uint64_t& term, const siteid_t& candidateId, const uint64_t& leaderCommitIndex, uint64_t* returnTerm) {
        rrr::Future* __fu__ = this->async_EmptyAppendEntries(term, candidateId, leaderCommitIndex);
        if (__fu__ == nullptr) {
            return ENOTCONN;
        }
        rrr::i32 __ret__ = __fu__->get_error_code();
